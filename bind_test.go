...
import (
	...
	tp "github.com/likearthian/types"
)

type TestDTO struct {
	MapField     tp.SliceOfString `query:"group"`
	CustomerCode string               `query:"customer_code"`
	CityName     tp.SliceOfString `query:"city_name"`
	ZipcodeID    tp.SliceOfString `query:"zipcode_id"`
	Zipcode      tp.SliceOfInt    `query:"zipcode"`
	Detailed     tp.Boolean        `query:"detailed"`
...
func TestEncodeToURLQuery(t *testing.T) {
	input := TestDTO{
		...
		Sort:         []Sort{{"zipcode", SORT_DESC},{"city_name", SORT_ASC}},
	}
...
func createTest1() (url.Values, TestDTO) {
	...
	dto := TestDTO{
		...
		Sort:     Sorts{{"field1", SORT_ASC}, {"field2", SORT_DESC}},
		Detailed: tp.BooleanFrom(true),
	}
